...
public class SpawnSkyRoamer extends Item{
	
	   private static final Map<EntityType<?>, ItemSpawnEgg> EGGS = Maps.newIdentityHashMap();
	   private final int primaryColor;
	   private final int secondaryColor;
	   private final EntityType<?> typeIn;
	   private final EntityType<?> sky =UnderGroundMod.EntitySkyRoamer;

	   public SpawnSkyRoamer(EntityType<?> typeIn, int primaryColorIn, int secondaryColorIn, Item.Properties builder) {
	      super(builder);
	      this.typeIn = typeIn;
	      this.primaryColor = primaryColorIn;
	      this.secondaryColor = secondaryColorIn;
	      ItemSpawnEgg e =new ItemSpawnEgg(typeIn,primaryColorIn,secondaryColorIn,builder);
	      EGGS.put(typeIn, e);
	   }
	   
	   public SpawnSkyRoamer(Item.Properties builder) {
		   super(builder);
		   this.typeIn=sky;
		   this.primaryColor=1;
	      ...
	      if (worldIn.isRemote) {
	         ...
	         if (raytraceresult != null && raytraceresult.type == RayTraceResult.Type.BLOCK) {
	            ...
	            if (!(worldIn.getBlockState(blockpos).getBlock() instanceof BlockFlowingFluid)) {
	               ...
	               EntityType<?> entitytype = this.getType(itemstack.getTag());
	               if (entitytype != null && sky.spawnEntity(worldIn, itemstack, playerIn, blockpos, false, false) != null) {
